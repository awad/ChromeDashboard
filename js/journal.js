...
app.View.Journals = Backbone.View.extend({
    ...
    render: function () {
        ...
        if (this.collection.length > 0) {
            app.View.todayJournal = new app.View.Journal({ model: this.collection.at(0) });
...
render: function() {
  ...
  save: function() {
      ...
      this.$el.fadeTo(500,0, function () {
          that.remove();
          app.collect.journals.create({ answer: val, day: 'today' });
      });
...
app.View.Journal = Backbone.View.extend({
  ...
  render: function() {
      var variables = { journal: this.model.get('answer'), day: this.model.get('day') };
